...
public class ProducerMessage implements RabbitTemplate.ConfirmCallback, RabbitTemplate.ReturnsCallback {
	...
	/**
	 * 如果发送到交换器都没有成功，比如删除了交换器，ack 返回值为 true， 如果发送到交换器成功，但是没有匹配的队列（比如取消了绑定），ack 返回值还是
	 * true.这是一个坑
	 *
	 ...
	 *
	 * 如果 ConfirmCallback 中 ack 返回 true，这里需要注意的是，当 exchange 没有找到匹配队列，ack 也会返回 true，说明消息发送到
	 * exchange 成功，这个时候需要查看 ReturnCallback 中有没有发生异常，如果消息正常投递，可以通过 correlationData 删除 redis
	 * 或者数据库中的本地消息 否则需要做异常处理
	 *
